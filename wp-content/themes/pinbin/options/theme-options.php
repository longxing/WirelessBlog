...
function pinbin_admin_options_page()
{
    ...
    <div class="wrap">
        ...
        <form id="form-pinbin-options" action="options.php" method="post" enctype="multipart/form-data">
            ...
            <div class="credit"><?php _e('You can reach us at', 'pinbin'); ?> <a
                        href="http://colorawesomeness.com/themes/pinbin/">Color Awesomeness</a>.
            </div>
...
function pinbin_setting_logo()
{
    $pinbin_options = get_option('theme_pinbin_options');
    ?>
    <input type="hidden" id="logo_url" name="theme_pinbin_options[logo]"
           value="<?php echo esc_url($pinbin_options['logo']); ?>"/>
    <input id="upload_logo_button" type="button" class="button" value="<?php _e('Upload Logo', 'pinbin'); ?>"/>
    <?php if ('' != $pinbin_options['logo']): ?>
    <input id="delete_logo_button" name="theme_pinbin_options[delete_logo]" type="submit" class="button"
           value="<?php _e('Delete Logo', 'pinbin'); ?>"/>
<?php endif; ?>
